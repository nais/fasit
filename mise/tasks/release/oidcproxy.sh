#!/usr/bin/env bash
#MISE description="Release oidcproxy"
set -euo pipefail
...
tag="oidcproxy-$(date +%Y%m%d%H%M%S)"
# If there's no changes locally, tag the release with `oidcproxy-[datetime]` and push it to origin
git tag -a "$tag" -m "Release oidcproxy"
