#!/usr/bin/env bash
#MISE description="Release fasitd"
set -euo pipefail
...
tag="fasitd-$(date +%Y%m%d%H%M%S)"
# If there's no changes locally, tag the release with `fasitd-[datetime]` and push it to origin
git tag -a "$tag" -m "Release fasitd"
