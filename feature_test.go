...
import (
	...
	feature2 "github.com/nais/fasit/pkg/feature2"
...
func TestFeatures_MappingValuesTemplate(t *testing.T) {
	source, err := feature.NewFeatureSourceFilesystem("./features", logrus.StandardLogger())
	if err != nil {
		t.Fatal(err)
	}
	defer source.Close()
	...
	for _, f := range mgr.Features() {
		t.Run(f.Name, func(t *testing.T) {
			for _, kind := range model.AllEnvironmentKind {
				...
				if err := feature2.Generate(f.Values, kind, mv, target); err != nil {
					t.Fatal(err)
				}
			}
		})
...
func TestFeatures_dependencies(t *testing.T) {
	...
	g := topsort.NewGraph()
	for _, f := range mgr.Features() {
		for _, deps := range f.Dependencies {
