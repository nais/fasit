...
func kubernetesNodeFromSQL(n gensql.KubernetesNodeStatus) (*model.KubernetesNode, error) {
	...
	if err := json.Unmarshal([]byte(n.Conditions), &conditions); err != nil {
		return nil, err
	}

	allocatable := &model.KubernetesNodeResources{}
	if err := json.Unmarshal([]byte(n.Allocatable), allocatable); err != nil {
		return nil, err
	}

	capacity := &model.KubernetesNodeResources{}
	if err := json.Unmarshal([]byte(n.Capacity), capacity); err != nil {
		return nil, err
	}
