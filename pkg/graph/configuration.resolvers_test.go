...
func Test_queryResolver_Configuration_With_Environment_ID(t *testing.T) {
	...
	repo.On("EnvironmentGet", ctx, envID).Return(&model.Environment{Kind: model.EnvironmentKindOnprem}, nil)
	...
	repo.On("ConfigGetForEnv", ctx, "feature", envID).Return(mockConfig, nil)
	...
	repo.On("ConfigGet", ctx, "feature").Return(mockGlobal, nil)

	r := &configurationsResolver{
		Resolver: &Resolver{
			Repo: repo,
		},
	}

	got, err := r.Configuration(ctx, &model.Configurations{
		FeatureName: "feature",
		EnvID:       &envID,
	})
