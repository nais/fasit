...
func TestDeactivation(t *testing.T) {
	...
	setupCtx := func(t *testing.T) (context.Context, *deploymenttest.Seeder) {
		t.Helper()
	...
	t.Run("creating deployment deactivates previous for same target", func(t *testing.T) {
		...
		dep1, err := deployment.Get(ctx, ids[0])
		if err != nil {
		...
		dep2, err := deployment.Get(ctx, ids[1])
		if err != nil {
	...
	t.Run("creating deployment does not deactivate different target", func(t *testing.T) {
		...
		seeder.AddDeployment("myapp", "1.0.0", environment.Labels{"kind": "tenant"})
		seeder.AddDeployment("myapp", "2.0.0", environment.Labels{"kind": "tenant", "name": "dev"})
		ids, err := seeder.Seed(ctx)
		if err != nil {
			t.Fatalf("seed: %v", err)
		}
	...
	t.Run("deactivate sets deployment inactive", func(t *testing.T) {
		...
		if err := deployment.Deactivate(ctx, ids[0]); err != nil {
			t.Fatalf("deactivate: %v", err)
		}

		dep, err := deployment.Get(ctx, ids[0])
		if err != nil {
			t.Fatalf("get: %v", err)
		}
		if dep.Active {
	...
	t.Run("ListByFeature returns only active deployments", func(t *testing.T) {
		...
		deps, err := deployment.ListByFeature(ctx, "myapp")
		if err != nil {
			t.Fatalf("list: %v", err)
		}
	...
	t.Run("ListAllByFeature returns active and inactive", func(t *testing.T) {
		...
		deps, err := deployment.ListAllByFeature(ctx, "myapp")
		if err != nil {
			t.Fatalf("list: %v", err)
		}
	...
	t.Run("deactivated deployment not visible in FeatureForEnvironment", func(t *testing.T) {
		...
		mgr.seeder.AddDeployment("myapp", "1.0.0", environment.Labels{"kind": "tenant"})
		ids, err := mgr.seeder.Seed(tCtx)
		if err != nil {
			t.Fatalf("seed: %v", err)
		}

		if err := deployment.Deactivate(tCtx, ids[0]); err != nil {
			t.Fatalf("deactivate: %v", err)
		}
		...
		_, err = deployment.FeatureForEnvironment(tCtx, env.ID, "myapp")
