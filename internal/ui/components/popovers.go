...
func RedeployPopover(popoverID, action, featureName, envName string, enabled bool) g.Node {
	if !enabled {
		return nil
	}
...
func ReconcilePopover(popoverID, action, featureName, envName string, enabled bool) g.Node {
	if enabled {
