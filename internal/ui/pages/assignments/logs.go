...
func LogsHandler(renderPage RenderPage) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		title := fmt.Sprintf("FeatureAssignment Logs: %s %s", dep.Feature.Name, dep.Feature.Version)
...
func logsPage(dep *featureassignment.FeatureAssignment, log *model.RolloutLog) g.Node {
	...
	content := []g.Node{
		h.H1(g.Textf("FeatureAssignment Logs: %s %s", dep.Feature.Name, dep.Feature.Version)),
	}
	...
	return h.Div(
		...
		components.Breadcrumbs([]breadcrumb.Crumb{
			...
			breadcrumb.FeatureAssignment(dep.ID.String(), dep.Feature.Name, dep.Feature.Version),
			{Label: "Logs"},
		}),
		h.Main(
