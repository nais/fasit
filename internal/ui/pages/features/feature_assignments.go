	...
	if prefs.Group == "assignment" && c.FeatureAssignmentID != "" {
		...
		actions = append(actions,
			h.Div(h.Class("card-kebab-wrap"),
				kebabButton("card-kebab-"+c.FeatureAssignmentID),
	...
	if showActions {
		...
		menuItems := []g.Node{
			h.A(h.Href(logsHref), h.Class("kebab-item"),
				g.Raw(`<svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M2 2h12v12H2V2zm1 1v10h10V3H3zm1 2h8v1H4V5zm0 2h8v1H4V7zm0 2h5v1H4V9z"/></svg> `),
				g.Text("Deploy logs"),
			),
			h.A(h.Href(environment.LokiExploreURL(env.TenantName, env.Name, featureName)), h.Class("kebab-item"), g.Attr("target", "_blank"), g.Attr("rel", "noopener"),
		...
		cells = append(cells, h.Td(h.Class("action"),
			h.Div(h.Class("kebab-wrap"),
				kebabButton(kebabID),
				h.Div(h.Class("kebab-menu"), h.ID(kebabID), g.Group(menuItems)),
