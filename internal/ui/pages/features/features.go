...
func overviewTab(data *DetailPage) g.Node {
	return g.Group([]g.Node{h.Pre(g.Text(data.CurrentFeature.Description+"\n"), g.Text("chart: "+data.CurrentFeature.Chart+"\n"), g.Text("version: "+data.CurrentFeature.Version+"\n"), g.Text("source: "), h.A(h.Href(data.CurrentFeature.Source), h.Target("_blank"), g.Text(data.CurrentFeature.Source)), g.Text("\n"), g.Text("dependencies:"), dependencyLinks(data.CurrentFeature)), h.Table(h.Class("table"), h.THead(h.Tr(h.Th(g.Text("Key")), h.Th(g.Text("Type")), h.Th(g.Text("Default Value")))), h.TBody(g.Group(g.Map(data.CurrentFeature.Config, func(item ConfigItem) g.Node { return h.Tr(h.Td(h.Span(h.Class("icon"), g.Text("📄")), g.Text(" "), g.Text(item.Key)), h.Td(g.Text(configTypeLabel(item))), h.Td(h.Class("italic"), configDefaultValue(item))) }))))})
}
...
func rolloutsTab(data *DetailPage) g.Node {
	if len(data.Rollouts) == 0 { return h.P(g.Text("No rollouts yet.")) }
...
func rolloutStatus(status string) g.Node { switch strings.ToUpper(status) { case "DEPLOYED": return g.Group([]g.Node{h.Span(h.Class("status-success"), g.Text("✓")), g.Text(" DEPLOYED")}); case "FAILED": return g.Group([]g.Node{h.Span(h.Class("status-error"), g.Text("✗")), g.Text(" FAILED")}); case "PENDING": return g.Group([]g.Node{h.Span(h.Class("status-pending"), g.Text("⏳")), g.Text(" PENDING")}); default: return g.Text(status) } }
func completedDate(value string) string { if value == "" { return "-" }; return value }
...
func loadFeatureData(r *http.Request, repo database.Repo, activeTab string) (*DetailPage, error) {
	...
	if activeTab == "status" { data.Environments = featureEnvironmentStatuses(r.Context(), repo, feature) }
	if activeTab == "rollouts" { data.Rollouts = featureRollouts(r.Context(), repo, featureName) }
	return data, nil
}
...
func featureConfigItems(feature *model.Feature) []ConfigItem {
	...
	for key, value := range feature.Values { if value.Config != nil { keys = append(keys, key) } }
	sort.Strings(keys)
	for _, key := range keys { value := feature.Values[key]; item := ConfigItem{Key: key, Value: rawValueToString(feature.ValuesYAML[key])}; if value.Config != nil { item.Type = strings.ToUpper(value.Config.Type.String()); item.IsSecret = value.Config.Secret }; if value.Computed != nil { item.IsComputed = true; item.Template = value.Computed.Template }; items = append(items, item) }
	return items
}
...
func featureEnvironmentStatuses(ctx context.Context, repo database.Repo, feature *model.Feature) []EnvironmentStatus {
	...
	for _, tenant := range tenants {
		...
		for _, env := range envs {
			if !featureTargetsKind(feature.EnvironmentKinds, env.Kind) { continue }
...
func featureTargetsKind(kinds []model.EnvironmentKind, envKind model.EnvironmentKind) bool { if len(kinds) == 0 { return true }; for _, kind := range kinds { if kind == envKind { return true } }; return false }
func configTypeLabel(item ConfigItem) string { if item.IsComputed { return "computed" }; if item.IsSecret { return "secret" }; if item.Type != "" { return strings.ToLower(item.Type) }; return "-" }
func configDefaultValue(item ConfigItem) g.Node { if item.IsSecret { return h.Span(h.Class("text-muted"), g.Text("[SECRET]")) }; if item.IsComputed { return h.Code(g.Text(item.Template)) }; if item.Value != "" { return h.Span(g.Text(item.Value)) }; return h.Span(h.Class("text-muted"), g.Text("-")) }
func toFeatureNavs(features []*model.Feature) []view.FeatureNav { ret := make([]view.FeatureNav, 0, len(features)); for _, feature := range features { ret = append(ret, view.FeatureNav{Name: feature.Name}) }; return ret }
func rawValueToString(value json.RawMessage) string { if len(value) == 0 { return "" }; var v any; if err := json.Unmarshal(value, &v); err != nil { return string(value) }; if s, ok := v.(string); ok { return s }; b, err := json.Marshal(v); if err != nil { return string(value) }; return string(b) }
var oslo = mustLoadLocation("Europe/Oslo")
func mustLoadLocation(name string) *time.Location { loc, err := time.LoadLocation(name); if err != nil { panic(err) }; return loc }
func formatTime(t time.Time) string { if t.IsZero() { return "" }; return t.In(oslo).Format("2006-01-02 15:04:05") }
func formatTimePtr(t *time.Time) string { if t == nil { return "-" }; return formatTime(*t) }
