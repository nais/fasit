...
CREATE OR REPLACE FUNCTION fasit_notify()
	RETURNS TRIGGER
	AS $$
BEGIN
...
BEGIN
	IF TG_NARGS > 0 THEN
		FOREACH key IN ARRAY TG_ARGV LOOP
			...
			VALUES
				:= array_append(
				VALUES, row_to_json(NEW) ->> key);
			ELSE
			VALUES
				:= array_append(
				VALUES, row_to_json(OLD) ->> key);
			END IF;
			i := i + 1;
		END LOOP;
	END IF;
	...
	-- }
	PERFORM
		pg_notify('fasit_notify', jsonb_build_object('table', TG_TABLE_NAME, 'op', TG_OP, 'data', jsonb_object(TG_ARGV,
				VALUES
))::TEXT);
	RETURN NULL;
	END;
...
CREATE TRIGGER configurations_global_notify
	AFTER INSERT OR UPDATE ON configurations_global
	FOR EACH ROW
	EXECUTE PROCEDURE fasit_notify("id", "feature");

CREATE TRIGGER configurations_environment_notify
	AFTER INSERT OR UPDATE ON configurations_environment
	FOR EACH ROW
